# rest-api-beispiele-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RestApiBeispiele SDK as
two agent tools — `rest-api-beispiele_list` and `rest-api-beispiele_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rest-api-beispiele-mcp)
make build

# 2. Provide credentials via the environment
export REST_API_BEISPIELE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user rest-api-beispiele \
  -- /absolute/path/to/rest-api-beispiele-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./rest-api-beispiele-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// rest-api-beispiele_list: first page of records
{ "entity": "delete" }
{ "entity": "delete", "query": { } }

// rest-api-beispiele_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rest-api-beispiele-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user rest-api-beispiele \
     -- "$PWD"/dist/*/rest-api-beispiele-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `rest-api-beispiele_list` and `rest-api-beispiele_load` tools now appear
   in new sessions. Ask the agent to *"list delete using rest-api-beispiele"*
   and it calls `rest-api-beispiele_list` with `{"entity":"delete"}`.
...
```sh
export REST_API_BEISPIELE_APIKEY=sk_live_xxx            # API key
export REST_API_BEISPIELE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `rest-api-beispiele_list` tool
...
### Call the `rest-api-beispiele_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `rest-api-beispiele_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `rest-api-beispiele_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REST_API_BEISPIELE_APIKEY` | API key sent with every request. |
| `REST_API_BEISPIELE_BASE` | Optional override of the API base URL. |
...
```sh
./rest-api-beispiele-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"rest-api-beispiele_load","arguments":{"entity":"product","query":{"id":1}}}}'
```
