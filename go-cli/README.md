...
boru-driven command-line client **and** interactive REPL for the RestApiBeispiele
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rest-api-beispiele-cli)
make build

# 2. See usage (words, entities, env vars)
./rest-api-beispiele-cli --help
...
REST_API_BEISPIELE_BASE=https://api.example.com ./rest-api-beispiele-cli --help

# 6. No arguments -> interactive REPL
./rest-api-beispiele-cli
rest-api-beispiele> /help
rest-api-beispiele> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rest-api-beispiele-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/rest-api-beispiele-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `rest-api-beispiele>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `delete`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/rest-api-beispiele-cli`. |
...
(`./rest-api-beispiele-cli <expr>`) and interactively (the REPL), and expressions compose the
