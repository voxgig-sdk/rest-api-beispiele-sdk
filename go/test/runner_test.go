...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("REST_API_BEISPIELE_TEST_LIVE") == "TRUE" ||
		os.Getenv("REST_API_BEISPIELE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("REST_API_BEISPIELE_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.RestApiBeispieleSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.RestApiBeispieleSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.RestApiBeispieleError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.RestApiBeispieleSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.RestApiBeispieleError{Msg: msg, Code: code}
}
