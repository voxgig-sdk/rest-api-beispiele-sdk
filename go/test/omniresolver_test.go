...
import (
	...
	omni "github.com/voxgig-sdk/rest-api-beispiele-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RestApiBeispieleError); is {
				out := map[string]any{
					"name":    "RestApiBeispieleError",
...
func omniCtx(arg any, client *sdk.RestApiBeispieleSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RestApiBeispieleSDK); is {
			client = live
