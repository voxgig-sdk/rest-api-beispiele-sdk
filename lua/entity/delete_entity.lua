...
local DeleteEntity = {}
DeleteEntity.__index = DeleteEntity


function DeleteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeleteEntity)
  self._name = "delete"
  self._client = client
...
function DeleteEntity:get_name()
  return self._name
end


function DeleteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeleteEntity.new(self._client, opts)
end
...
function DeleteEntity:mark_deleted()
  self._deleted = true
end


function DeleteEntity:deleted()
  return true == self._deleted
end


function DeleteEntity:data_set(args)
  if args ~= nil then
...
function DeleteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeleteEntity:match_set(args)
  if args ~= nil then
...
function DeleteEntity:match_get()
...
function DeleteEntity:stream(action, args, callopts)
...
function DeleteEntity:_run_op(ctx, post_done)
  local utility = self._utility
