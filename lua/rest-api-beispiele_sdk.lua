...
require("rest-api-beispiele_types")
...
local RestApiBeispieleSDK = {}
RestApiBeispieleSDK.__index = RestApiBeispieleSDK
...
RestApiBeispieleSDK._make_feature = _make_feature


function RestApiBeispieleSDK.new(options)
  local self = setmetatable({}, RestApiBeispieleSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RestApiBeispieleSDK:options_map()
...
function RestApiBeispieleSDK:get_utility()
  return Utility.copy(self._utility)
end


function RestApiBeispieleSDK:get_root_ctx()
  return self._rootctx
end


function RestApiBeispieleSDK:prepare(fetchargs)
...
function RestApiBeispieleSDK:direct(fetchargs)
...
function RestApiBeispieleSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function RestApiBeispieleSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "RestApiBeispieleSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function RestApiBeispieleSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RestApiBeispieleSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RestApiBeispieleSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RestApiBeispieleSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RestApiBeispieleSDK.new(sdkopts)
  sdk.mode = "test"
