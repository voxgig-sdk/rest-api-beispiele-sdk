...
from projectname_sdk.core.control import RestApiBeispieleControl
...
class RestApiBeispieleContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = RestApiBeispieleControl()
        ...
        if isinstance(sp, RestApiBeispieleSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, RestApiBeispieleResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, RestApiBeispieleResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return RestApiBeispieleOperation({})
    ...
    def make_error(self, code, msg):
        return RestApiBeispieleError(code, msg, self)
