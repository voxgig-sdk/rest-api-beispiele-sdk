...
//    only - a typed RestApiBeispieleContext there would make every ctx
...
final class RestApiBeispieleOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RestApiBeispieleContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new RestApiBeispieleSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new RestApiBeispieleResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new RestApiBeispieleError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new RestApiBeispieleResponse($clean['response']);
    ...
    public static function err_from_map($m): ?RestApiBeispieleError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RestApiBeispieleError(is_string($code) ? $code : '', $msg);
    }
