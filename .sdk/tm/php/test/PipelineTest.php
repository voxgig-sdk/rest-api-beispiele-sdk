...
// RestApiBeispiele SDK pipeline test
//
...
{
    private static function utility(): RestApiBeispieleUtility
    {
        return new RestApiBeispieleUtility();
    }
    ...
    private static function ctx(array $over = []): RestApiBeispieleContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RestApiBeispieleContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RestApiBeispieleOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RestApiBeispieleError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RestApiBeispieleOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RestApiBeispieleMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RestApiBeispieleOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RestApiBeispieleMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RestApiBeispieleOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RestApiBeispieleMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RestApiBeispieleSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RestApiBeispieleMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RestApiBeispieleResponse([]),
            'result' => new RestApiBeispieleResult([]),
        ]);
        [, $err] = RestApiBeispieleMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RestApiBeispieleSpec([]),
            'response' => null,
            'result' => new RestApiBeispieleResult([]),
        ]);
        [, $err] = RestApiBeispieleMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RestApiBeispieleSpec(['step' => 's']),
            'response' => new RestApiBeispieleResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RestApiBeispieleResult([]),
        ]);
        [, $err] = RestApiBeispieleMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RestApiBeispieleSpec(['step' => 's']),
            'response' => new RestApiBeispieleResponse(self::resp(200, ['v' => 1])),
            'result' => new RestApiBeispieleResult([]),
        ]);
        [, $err] = RestApiBeispieleMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RestApiBeispieleSpec(['step' => 's']),
            'response' => new RestApiBeispieleResponse(self::resp(200, ['v' => 2])),
            'result' => new RestApiBeispieleResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RestApiBeispieleMakeResponse::call($ctx);
    ...
    {
        $preset = new RestApiBeispieleResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RestApiBeispieleSpec([]),
            'response' => new RestApiBeispieleResponse([]),
            'result' => new RestApiBeispieleResult([]),
        ...
        [$got, $err] = RestApiBeispieleMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RestApiBeispieleResult([])]);
        [, $err] = RestApiBeispieleMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RestApiBeispieleSpec([]), 'result' => null]);
        [, $err] = RestApiBeispieleMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RestApiBeispieleSpec(['step' => 's']),
            'result' => new RestApiBeispieleResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RestApiBeispieleOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RestApiBeispieleMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RestApiBeispieleSpec(['step' => 's']),
            'result' => new RestApiBeispieleResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RestApiBeispieleOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RestApiBeispieleMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RestApiBeispieleResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RestApiBeispieleSpec([]),
            'result' => new RestApiBeispieleResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RestApiBeispieleMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RestApiBeispieleMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RestApiBeispieleError('boom', 'boom');
        $utility->fetcher = function (RestApiBeispieleContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RestApiBeispieleSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RestApiBeispieleMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RestApiBeispieleContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RestApiBeispieleMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RestApiBeispieleResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RestApiBeispieleMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RestApiBeispieleContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RestApiBeispieleSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RestApiBeispieleMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RestApiBeispieleResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RestApiBeispieleSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RestApiBeispieleMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RestApiBeispieleMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RestApiBeispieleMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RestApiBeispieleResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RestApiBeispieleDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RestApiBeispieleResult(['ok' => false])]);
        $this->expectException(RestApiBeispieleError::class);
        RestApiBeispieleDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RestApiBeispieleResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RestApiBeispieleMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RestApiBeispieleMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RestApiBeispieleBaseFeature();
        $b = new RestApiBeispieleBaseFeature();
        RestApiBeispieleFeatureAdd::call($ctx, $a);
        RestApiBeispieleFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RestApiBeispieleBaseFeature
    {
        $f = new RestApiBeispieleBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RestApiBeispieleFeatureAdd::call($ctx, self::named_feature('a'));
        RestApiBeispieleFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        RestApiBeispieleFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        RestApiBeispieleFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        RestApiBeispieleFeatureAdd::call($ctx, $replace);
        ...
        RestApiBeispieleFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RestApiBeispieleContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RestApiBeispieleSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RestApiBeispielePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RestApiBeispielePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RestApiBeispieleResponse(['headers' => null]),
            'result' => new RestApiBeispieleResult([]),
        ]);
        RestApiBeispieleResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RestApiBeispieleResultBody::call($ctx);
