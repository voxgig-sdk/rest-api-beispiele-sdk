...
// RestApiBeispiele SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RestApiBeispieleOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RestApiBeispieleOmni::structfix($now, Struct::undef()))
                        !== json_encode(RestApiBeispieleOmni::structfix($val, Struct::undef()))) {
