...
class TestDeleteEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Delete(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _delete_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "delete." + _op, "live" if _live else "unit")
            if _skip:
...
def _delete_basic_setup(extra):
    ...
    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/delete/DeleteTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "REST_API_BEISPIELE_TEST_DELETE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("REST_API_BEISPIELE_TEST_DELETE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("REST_API_BEISPIELE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("REST_API_BEISPIELE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("REST_API_BEISPIELE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
