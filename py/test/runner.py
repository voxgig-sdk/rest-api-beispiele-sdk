# RestApiBeispiele SDK test runner
...
class RestApiBeispieleTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RestApiBeispieleTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RestApiBeispieleTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RestApiBeispieleTestRunner.getenv("REST_API_BEISPIELE_TEST_LIVE")
        override = RestApiBeispieleTestRunner.getenv("REST_API_BEISPIELE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RestApiBeispieleTestRunner.getenv(key)
        ...
        explain = RestApiBeispieleTestRunner.getenv("REST_API_BEISPIELE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RestApiBeispieleTestRunner._test_control is not None:
            return RestApiBeispieleTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RestApiBeispieleTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RestApiBeispieleTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RestApiBeispieleTestRunner.load_test_control()
...
def load_env_local():
    RestApiBeispieleTestRunner.load_env_local()


def env_override(m):
    return RestApiBeispieleTestRunner.env_override(m)


def entity_data(v):
    return RestApiBeispieleTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RestApiBeispieleTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RestApiBeispieleTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RestApiBeispieleTestRunner.load_test_control()


def live_client_options():
    return RestApiBeispieleTestRunner.live_client_options()


def live_delay_ms():
    return RestApiBeispieleTestRunner.live_delay_ms()
