...
//     (\`new RestApiBeispieleSDK(...)\` / \`RestApiBeispieleSDK.test(...)\` become
//     \`RestApiBeispieleSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RestApiBeispieleSDK'
...
const TEST_SEED = {"entity":{"delete":{"test01":{"id":"test01"}},"product":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RestApiBeispieleSDK(...)\` and any
// \`RestApiBeispieleSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RestApiBeispieleSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RestApiBeispieleSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
