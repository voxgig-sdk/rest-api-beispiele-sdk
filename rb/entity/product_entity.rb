...
class ProductEntity
  ...
  def data_set(args)
    if args
      @_data = RestApiBeispieleHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = RestApiBeispieleHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RestApiBeispieleHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Product, Hash] the created Product; raises RestApiBeispieleError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Product, Hash] the updated Product; raises RestApiBeispieleError on failure
