...
module RestApiBeispieleUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RestApiBeispieleResponse.new({})
    ...
    if fetch_err
      ...
      response = RestApiBeispieleResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RestApiBeispieleResponse.new(fetched)
    else
