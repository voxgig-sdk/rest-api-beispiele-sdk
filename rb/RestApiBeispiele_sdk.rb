...
require_relative 'RestApiBeispiele_types'


class RestApiBeispieleSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RestApiBeispieleHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RestApiBeispieleHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RestApiBeispieleFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RestApiBeispieleUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RestApiBeispieleHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RestApiBeispieleHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RestApiBeispieleHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = RestApiBeispieleSpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RestApiBeispieleError.new(
        "#{op}_allow",
        "RestApiBeispieleSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RestApiBeispieleError => err
    ...
    if fetched.is_a?(Hash)
      status = RestApiBeispieleHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RestApiBeispieleError.new(
        "graphql_error", "RestApiBeispieleSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Delete.list / client.Delete.load({ "id" => ... })
  def Delete(data = nil)
    require_relative 'entity/delete_entity'
    DeleteEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RestApiBeispieleSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
