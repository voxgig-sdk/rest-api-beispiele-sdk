...
class RestApiBeispieleSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = RestApiBeispieleHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RestApiBeispieleHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RestApiBeispieleFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, RestApiBeispieleFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return RestApiBeispieleUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = RestApiBeispieleHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = RestApiBeispieleHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = RestApiBeispieleHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RestApiBeispieleError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = RestApiBeispieleHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RestApiBeispieleError("graphql_error",
                "RestApiBeispieleSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Delete($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_delete === null) {
                $this->_delete = new DeleteEntity($this, null);
            }
            return $this->_delete;
        }
        return new DeleteEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new RestApiBeispieleSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
