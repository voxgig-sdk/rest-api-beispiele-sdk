...
// RestApiBeispiele SDK feature test
//
...
class FtCtrl extends RestApiBeispieleControl
{
...
class FtClient
{
    ...
    private RestApiBeispieleUtility $utility;

    public function __construct(RestApiBeispieleUtility $utility, array $options)
    {
    ...
    public function get_utility(): RestApiBeispieleUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RestApiBeispieleContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RestApiBeispieleContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RestApiBeispieleContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RestApiBeispieleConfig::make_feature($name);
    ...
    private static function buildUrl(RestApiBeispieleSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RestApiBeispieleError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RestApiBeispieleResponse($fetched);
        }
    ...
    private function fail(RestApiBeispieleContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RestApiBeispieleContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
