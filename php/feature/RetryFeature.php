...
// RestApiBeispiele SDK retry feature
...
class RestApiBeispieleRetryFeature extends RestApiBeispieleBaseFeature
{
    ...
    public function init(RestApiBeispieleContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RestApiBeispieleContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RestApiBeispieleContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RestApiBeispieleContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
