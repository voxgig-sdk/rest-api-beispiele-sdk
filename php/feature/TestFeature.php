...
// RestApiBeispiele SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RestApiBeispieleTestFeature extends RestApiBeispieleBaseFeature
{
    ...
    public function init(RestApiBeispieleContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RestApiBeispieleContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RestApiBeispieleParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RestApiBeispieleContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RestApiBeispieleContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RestApiBeispieleParam::call($ctx, $k);
